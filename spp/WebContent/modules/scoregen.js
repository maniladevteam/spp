var param_prodfam = "('Legacy AIM','Eikon','Transactions','Other Products/Sunset','Datascope','Admin')";
...
var updatedate = "";
	...
	$(document).ready(function() {
					...
					$("#extractrawdatabut").click(function() {							
							GetSppRawData($("#filter_view_rawmodal").val(), "", $("#freqno_rawmodal").val(), $("#yearno_rawmodal").val());
			    	...
			    	$('#crossmodularbut').click(function () {
			    		...
			    		GetUserAuditLog($("#uidlogged").val(), $("#user_role").text(), "View Cross Modular", $("#user_name").text(),'');
	...
	$.ajax({
		...
		success : function(response) {
			...
			$("#updatedate").html("");
				...
				updatedate =  "<span class='pull-right'><!-- Note from the main chart box --></span> Last Update : " + resultsArray[0].updatedate;
...
function RunScoreCm(param_view, param_yearno){
	...
	GetSppCmScore($("#filter_view_cm").val(), $("#freqno_cm").val(), $("#yearno_cm").val());
	GetCmScore_TimeBucket($("#filter_view_cm").val(), $("#freqno_cm").val(), $("#yearno_cm").val());
}
...
function GetOverallSppScore(param_view, param_freqno, param_yearno) {
	...
	function GetSppScore_Chart_Cur(param_view, param_freqno, param_yearno) {
		...
		$.ajax({
			...
			complete : function(e) {
				 RenderChartJsData();
				 GetGamefySpScore(param_view, param_freqno, param_yearno)
			}

		});
	}
	
	function GetGamefySpScore(param_view, param_freqno, param_yearno) {
...
function GetProdLcSppScore_TimeBucket(param_view, param_freqno, param_yearno, param_prodlclist) {
	...
	function RenderSppScoreTimeBucket() {
		...
		dataTable = dataTable + "<th style='width: 100px;'>Functions</th>"
		dataTable = dataTable + "<th style='width: 50px;'>30 mins below</th>"
		dataTable = dataTable + "<th style='width: 50px;'>30 - 60 mins</th>"
		dataTable = dataTable + "<th style='width: 50px;'>1 - 2 hrs</th>"
		dataTable = dataTable + "<th style='width: 50px;'>2 - 4 hrs</th>"
		dataTable = dataTable + "<th style='width: 50px;'>4 - 8 hrs</th>"
		dataTable = dataTable + "<th style='width: 50px;'>8 - 24 hrs</th>"
		dataTable = dataTable + "<th style='width: 50px;'>Beyond 24 hrs</th>"
...
function GetProdLcSppScore(param_view, param_freqno, param_yearno, param_prodlc) {
	...
	function RenderProdLcSppScore() {
		...
		$("#prodlc_totalsurveys").append(dataTable2);
...
function GetWgSppScore(param_view, param_freqno, param_yearno, param_workgroup) {
	...
	function RenderWgSppScore() {
		...
		$("#wg_totalsurveys").append(dataTable2);
...
function GetSppCmScore(param_view, param_freqno, param_yearno) {
	...
	GetSppCmScore_Cur(param_view, param_freqno, param_yearno);
	
	function GetSppCmScore_Cur(param_view, param_freqno, param_yearno) {
		...
		$.ajax({
			type : "POST",
			url : "spp-admin/action-spp/sppcmscore",
			data : "{\"param_view\":" + '"' + SelectView(param_view,1)
					+ "\",\"param_freqno\":" + '"' + param_freqno
					+ "\",\"param_yearno\":" + '"' + param_yearno + '"}',
			contentType : "application/x-www-form-urlencoded",
			...
			complete : function(e) {
				...
				GetSppCmScore_Pre(param_view, previous_freqno,param_yearno);
	...
	function GetSppCmScore_Pre(param_view, param_freqno, param_yearno) {
		...
		$.ajax({
			...
			success : function(response) {
				...
				RenderSppCmScore()
				GetSppCmScore_Chart_Cur(param_view, param_freqno, param_yearno)
			}

		});
	}
	
	function GetSppCmScore_Chart_Cur(param_view, param_freqno, param_yearno) {
...
function GetCmScore_TimeBucket(param_view, param_freqno, param_yearno) {
	...
	GetSppTimeBucket_List(param_view, param_freqno, param_yearno);

	function GetSppTimeBucket_List(param_view, param_freqno, param_yearno) {

		var i = 0;

		$.ajax({
					type : "POST",
					url : "spp-admin/action-spp/sppscorecmtimebucket", 
					data : "{\"param_view\":" + '"' + SelectView(param_view,1)
							+ "\",\"param_freqno\":" + '"' + param_freqno
							+ "\",\"param_yearno\":" + '"' + param_yearno + '"}',
					contentType : "application/x-www-form-urlencoded",
					...
					success : function(response) {
						...
						for (var i = 0; i < resultsArray.length; i++) {
							rownum = resultsArray.length;
							target_mapping[i] = resultsArray[i].target_mapping;
							tp2count_30below[i] = resultsArray[i].tp2_30below_count;
							tp2count_30_60[i] = resultsArray[i].tp2_30_60_count;
							tp2count_1_2hrs[i] = resultsArray[i].tp2_1_2hrs_count;
							tp2count_2_4hrs[i] = resultsArray[i].tp2_2_4hrs_count;
	...
	function RenderSppScoreTimeBucket() {
		...
		for (var i = 0; i < rownum; i++) {
			...
			dataTable = dataTable + "<tr><td>" + target_mapping[i] + "</td>"
			dataTable = dataTable + "<td class='text-center'>"+ tp2total_30below[i] +" - " + (ZeroDivide(tp2count_30below[i], tp2total_30below[i]) * 100).toFixed(0) + "%</td>"
			dataTable = dataTable + "<td class='text-center'>"+ tp2total_30_60[i] +" - " + (ZeroDivide(tp2count_30_60[i], tp2total_30_60[i]) * 100).toFixed(0) + "%</td>"
			dataTable = dataTable + "<td class='text-center'>"+ tp2total_1_2hrs[i] +" - " + (ZeroDivide(tp2count_1_2hrs[i], tp2total_1_2hrs[i]) * 100).toFixed(0) + "%</td>"
			dataTable = dataTable + "<td class='text-center'>"+ tp2total_2_4hrs[i] +" - " + (ZeroDivide(tp2count_2_4hrs[i], tp2total_2_4hrs[i]) * 100).toFixed(0) + "%</td>"
...
function GetHdRaw(){
	...
	window.location.href = "spprawadmin?param_view="+ SelectView($('#filter_view_rawmodal').val(),1) +"&param_fxngroup=&param_freqno="+ $('#freqno_rawmodal').val() +"&param_yearno=" + $("#yearno_rawmodal").val();
