...
var param_serialno = "";
	...
	$(document).ready(function() {
					...
					$("#extractrawdatabut").click(function() {							
							GetSppRawData($("#filter_view_rawmodal").val(), "Content", $("#freqno_rawmodal").val(), $("#yearno_rawmodal").val());
...
function GetWgScore_TimeBucket(param_view, param_freqno, param_yearno, param_prodlc) {
	...
	function RenderSppScoreTimeBucket() {
		...
		for (var i = 0; i < rownum; i++) {
			...
			dataTable = dataTable + "<tr><td>" + (case_owner_role[i]).substring(4) + "</td>"
...
function GetWgSppScore_list(param_view, param_freqno, param_yearno, param_prodlc) {
	...
	function RenderSppScore() {
		...
		for (var i = 0; i < rownum; i++) {
			...
			dataTable = dataTable + "<a href='#'><h5 class='no-margins font-bold text-color3'>"+ (case_owner_role[i]).substring(4) + "</h5></a>"
			dataTable = dataTable + "</div>"
...
function GetHdRaw(){
	...
	window.location.href = "spprawadmin?param_view="+ SelectView($('#filter_view_rawmodal').val(),1) +"&param_fxngroup=Content&param_freqno="+ $('#freqno_rawmodal').val() +"&param_yearno=" + $("#yearno_rawmodal").val();
