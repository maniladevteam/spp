...
public class SppTmRaw extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String param_tm_id = request.getParameter("param_tm_id");
		String param_empid = request.getParameter("param_empid");
		String param_accesskey = request.getParameter("param_accesskey");
		...
		try {
			...
			String sql = "call spp.spp_rawdata_tm_download(?,?,?,?,?,?)";
			...
			query.setString(6, param_accesskey);

			java.sql.ResultSet sppraw = query.executeQuery();
			java.sql.ResultSet spprawtwo = sppraw;

			while (sppraw.next()) {
				valid = sppraw.getInt("valid");
			...
			if (valid == 1) {
				...
				response.setHeader("Content-Disposition",
						"attachment; filename=SppRaw_" + param_view + "_" + param_freqno + "_" + param_yearno + ".xls");
			...
			Logger.getLogger(SppTmRaw.class.getName()).log(Level.SEVERE, null, ex);
	...
	private void UpdateKey(String key) {
		...
		try {
			sppbean.UserAccessKeyValid(key);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (Exception e) {
